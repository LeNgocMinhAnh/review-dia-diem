...
export default function ReviewForm({ visible, place, hideRatingShow }) {
  ...
  const submitReview = useCallback(
    async (event) => {
      ...
      // Upload hình
      if (images.length > 0) {
        const { data: data2, status: uploadStatus } = await uploadImages(
          images
        );
        if (uploadStatus === 201) {
  ...
  return (
    <div
      className={
        visible
          ? " transition-all duration-500 fixed backdrop-opacity-50 bg-black bg-opacity-70 inset-0 z-99"
          : "hidden"
      }
    >
      <div className="flex justify-center h-screen items-center antialiased">
        <div className=" flex bg-gray-50 flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl">
          <div className="flex flex-row justify-between px-6 py-4 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
            <span className="font-semibold text-gray-600 w-full truncate">
              Review{" "}
          ...
          <div className="flex overflow-y-auto form-content flex-col px-6 bg-gray-50">
            ...
            <div className="flex items-center justify-between select-none">
              <div className="flex items-center mb-3 space-x-2">
                ...
                />
                <label
                  className="inline-flex font-semibold text-gray-700 cursor-pointer"
                  htmlFor="check1"
                >
                  Đăng ẩn danh
                </label>
              ...
              <div
                onClick={onClickAttachImage}
                className="flex items-center mb-3 space-x-1 text-blue-600 hover:text-blue-900"
              >
                <svg
                ...
                >
                  <path
                    d="M464.059,61.567c-63.811-63.814-167.646-63.814-231.468,0L34.183,259.974c-45.579,45.584-45.576,119.753,0.006,165.334
          ...
          <div className="flex flex-row items-center justify-between py-4 px-6 bg-white border-t border-gray-200 rounded-bl-lg rounded-br-lg">
            <p
              onClick={hideRatingShow}
            ...
            <button
              ...
              className="px-4 py-2 flex items-center text-white font-semibold bg-blue-500 rounded"
            >
              {isLoading ? (
                <svg
